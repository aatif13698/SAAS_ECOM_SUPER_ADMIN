...
const Navmenu = ({ menus }) => {
  ...
  useEffect(() => {
    const adminInfo = localStorage.getItem("saas_adminInfo");
    const parseInfo = JSON.parse(adminInfo);
    const roleId = parseInfo?.roleId;
    if (roleId == 1) {
      // console.log("calling this menu");
      
      setSideNavItems(menus);
    } else if (roleId == 2) {
