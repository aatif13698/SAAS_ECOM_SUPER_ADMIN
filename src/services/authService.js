...
const Login = async (data) => {
  ...
  return await axios
    .post(`${import.meta.env.VITE_BASE_URL}/api/superAdmin/signIn`, data)
    .then((response) => {
      return response;
    });
};
...
const OtpSignIn = async (data) => {
  try {
    const response = await axios.post(
      `${import.meta.env.VITE_BASE_URL}/api/superAdmin/signInByOtp`,
      data
    );
    return response;
  } catch (error) {
...
export default { Login, OtpSignIn, getProfile, updateProfile,forgotPassword,resetPassword };
