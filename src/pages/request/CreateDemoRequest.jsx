...
function CreateDemoRequest() {
  ...
  useEffect(() => {
    if (client && pathName === "/view/request") {
      const fetchReplies = async () => {
        try {
          const response = await requestService.getParticularRequest(client._id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (client) {
        // response = await requestService.updateRequest(client._id, dataObject);
      } else {
        response = await requestService.createRequest(dataObject);
      }
      setFormData({
        name: "",
        email: "",
  ...
  const handleReplySubmit = async (e) => {
    ...
    try {
      ...
      const response = await requestService.addReply(client._id, replyObject);
